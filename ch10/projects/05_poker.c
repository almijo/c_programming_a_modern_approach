...
void analyze_hand(void) {
    ...
    int lowest_rank = 0;
    while (num_in_rank[lowest_rank] == 0)
        lowest_rank++;

    // check for straight
    for (rank = lowest_rank; rank < NUM_RANKS && num_in_rank[rank] > 0; rank++)
        num_consec++;
    if (num_consec == NUM_CARDS ||
        num_consec == NUM_CARDS - 1 && lowest_rank == 0 && num_in_rank[NUM_RANKS - 1] == 1) {
        straight = true;
        return;
    }
